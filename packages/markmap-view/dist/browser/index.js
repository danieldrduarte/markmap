(function(exports, d32) {
  ...
  class Markmap {
    constructor(svg, opts) {
      ...
      this._disposeList = [];
      this._sideMap = /* @__PURE__ */ new Map();
      this.handleZoom = (e) => {
        const { transform } = e;
    ...
    async toggleSide(data, side) {
      var _a;
      const foldKey = side === "left" ? "foldLeft" : "foldRight";
      const current = (_a = data.payload) == null ? void 0 : _a[foldKey];
    ...
    _relayout() {
      ...
      this.g.selectAll(childSelector(SELECTOR_NODE)).selectAll(
        ...
        var _a2;
        const el = (_a2 = this.firstChild) == null ? void 0 : _a2.firstChild;
      ...
      const makeLayout = () => flextree({}).children((d) => {
        var _a2;
        if (!((_a2 = d.payload) == null ? void 0 : _a2.fold)) return d.children;
      }).nodeSize((node) => {
      ...
      const rightChildren = ((_a = root.payload) == null ? void 0 : _a.foldRight) ? [] : allChildren.filter((_, i) => i % 2 === 0);
      const leftChildren = ((_b = root.payload) == null ? void 0 : _b.foldLeft) ? [] : allChildren.filter((_, i) => i % 2 !== 0);
      ...
      this.state.rect = {
        x1: d32.min(allFnodes, (fnode) => fnode.data.state.rect.x) || 0,
        y1: d32.min(allFnodes, (fnode) => fnode.data.state.rect.y) || 0,
        x2: d32.max(
          allFnodes,
          (fnode) => fnode.data.state.rect.x + fnode.data.state.rect.width
        ) || 0,
    ...
    async renderData(originData) {
      ...
      walkTree(rootNode, (item, next, parent) => {
        ...
        if (this.options.bidirectional && parent === rootNode) {
          ...
          const isFolded = side === "left" ? (_a = rootNode.payload) == null ? void 0 : _a.foldLeft : (_b = rootNode.payload) == null ? void 0 : _b.foldRight;
          if (isFolded) return;
        }
        if (!((_c = item.payload) == null ? void 0 : _c.fold)) next();
      ...
      const getCircleData = (d) => {
        var _a;
        if (!((_a = d.children) == null ? void 0 : _a.length)) return [];
      ...
      const mmCircleMerge = mmCircleEnter.merge(mmCircle).attr("stroke", (d) => color(d.node)).attr("fill", (d) => {
        var _a, _b;
        if (this.options.bidirectional && !this._sideMap.has(d.node.state.id) && d.side !== null) {
          const foldKey = d.side === "left" ? "foldLeft" : "foldRight";
          return ((_a = d.node.payload) == null ? void 0 : _a[foldKey]) ? color(d.node) : "var(--markmap-circle-open-bg)";
        }
        return ((_b = d.node.payload) == null ? void 0 : _b.fold) && d.node.children ? color(d.node) : "var(--markmap-circle-open-bg)";
      });
      ...
      const links = nodes.flatMap((node) => {
        var _a;
        if ((_a = node.payload) == null ? void 0 : _a.fold) return [];
        if (this.options.bidirectional && !this._sideMap.has(node.state.id)) {
          return (node.children ?? []).flatMap((child) => {
            var _a2, _b;
            const side = this._sideMap.get(child.state.id);
            const isFolded = side === "left" ? (_a2 = node.payload) == null ? void 0 : _a2.foldLeft : (_b = node.payload) == null ? void 0 : _b.foldRight;
      ...
      const mmPathEnter = mmPath.enter().insert("path", "g").attr("class", "markmap-link").attr("data-depth", (d) => d.target.state.depth).attr("data-path", (d) => d.target.state.path).attr("d", (d) => {
        ...
        const isLeft = this.options.bidirectional && this._sideMap.get(d.target.state.id) === "left";
      ...
      mmGEnter.attr("transform", (d) => {
        ...
        const isLeft = this.options.bidirectional && this._sideMap.get(d.state.id) === "left";
        const x = isLeft ? originRect.x : originRect.x + originRect.width - d.state.rect.width;
      ...
      this.transition(mmGExit).attr("transform", (d) => {
        ...
        const isLeft = this.options.bidirectional && this._sideMap.get(d.state.id) === "left";
        const targetX = isLeft ? targetRect.x : targetRect.x + targetRect.width - d.state.rect.width;
      ...
      mmCircleMerge.attr("cx", (d) => d.side === "left" ? 0 : d.node.state.rect.width).attr("cy", (d) => d.node.state.rect.height + lineWidth(d.node) / 2);
      ...
      this.transition(mmPathMerge).attr("stroke", (d) => color(d.target)).attr("stroke-width", (d) => lineWidth(d.target)).attr("d", (d) => {
        ...
        const isLeft = this.options.bidirectional && this._sideMap.get(origTarget.state.id) === "left";
        const source = [
          isLeft ? origSource.state.rect.x : origSource.state.rect.x + origSource.state.rect.width,
          origSource.state.rect.y + origSource.state.rect.height + lineWidth(origSource) / 2
        ];
        const target = [
          isLeft ? origTarget.state.rect.x + origTarget.state.rect.width : origTarget.state.rect.x,
